...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pageSize = floatval($_POST['pageSize'] ?? 0);
    ...
    // Validate inputs
    if ($pageSize <= 0 || $imageCount < 0) {
        http_response_code(400);
        echo json_encode(['error' => 'Invalid input values']);
        exit;
    }

    // Calculate base load time (simplified calculation)
    $baseLoadTime = calculateBaseLoadTime($pageSize, $imageCount, $optimizationLevel);
    
    // Adjust for connection speed
    $speedFactor = getConnectionSpeedFactor($connectionSpeed);
    $adjustedLoadTime = $baseLoadTime * $speedFactor;
    
    // Adjust for geographic distance
    $distanceFactor = getDistanceFactor($serverLocation, $userLocation);
    $finalLoadTime = $adjustedLoadTime * $distanceFactor;

    // Calculate performance score (0-100)
    $performanceScore = calculatePerformanceScore($finalLoadTime);
    
    // Get performance grade
    $performanceGrade = getPerformanceGrade($performanceScore);
    
    // Get recommendations
    $recommendations = generateRecommendations($finalLoadTime, $optimizationLevel, $pageSize, $imageCount);

    // Prepare response
    $response = [
        'success' => true,
        'loadTime' => round($finalLoadTime, 2) . 's',
...
function calculateBaseLoadTime($pageSize, $imageCount, $optimizationLevel) {
    $baseTime = $pageSize * 0.8; // Base time based on page size
    
    // Add time for images
    $imageTime = $imageCount * 0.1;
    ...
    return ($baseTime + $imageTime) * $factor;
}

function getConnectionSpeedFactor($speed) {
    $factors = [
        'slow' => 2.5,
        'medium' => 1.0,
        'fast' => 0.6,
...
function getDistanceFactor($serverLoc, $userLoc) {
